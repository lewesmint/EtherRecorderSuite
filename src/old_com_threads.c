...
#define OLD_BLOCK_SIZE 4                      // Each column holds 4 bytes.
#define OLD_BLOCK_CHAR_COUNT (OLD_BLOCK_SIZE * 2) // 4 bytes ? 8 characters.
#define OLD_COL_WIDTH (BLOCK_CHAR_COUNT + 1)  // Plus one space separator.
...
static OLD_config_t config = { .cols = 4, .start = 0 };
 ...
 */
static void OLD_init_row(char* row, int cols) {
 ...
 */
static void OLD_log_buffered_data(uint8_t* buffer, int* buffered_length, int batch_bytes) {
 ...
 */
void OLD_setup_select_timeout(SOCKET* sock, fd_set* write_fds, struct timeval* timeout, long sec, long usec) {
    FD_ZERO(write_fds);
 ...
 */
static bool OLD_handle_data_reception(SOCKET sock, char* buffer, int* buffered_length, int* batch_bytes) {
...
void* OLD_receive_thread(void* arg) {
