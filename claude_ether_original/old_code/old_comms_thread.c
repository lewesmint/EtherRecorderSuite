 ...
 */
static CommContext* validate_comm_args(AppThread_T* thread_info) {
    CommContext* comm_args = (CommContext*)thread_info->data;
 ...
 * @return true if successful, false if should exit
 */
static bool handle_received_data(SOCKET sock, CommContext* comm_args, CommsThreadArgs_T* comms_info,
...
void* receive_thread_function(void* arg) {
    ...
    CommContext* comm_args = validate_comm_args(thread_info);
    if (!comm_args) {
        return NULL;
    }
