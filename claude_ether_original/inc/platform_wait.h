 ...
 * @return PlatformWaitResult Platform-agnostic wait result (PLATFORM_WAIT_SUCCESS, PLATFORM_WAIT_TIMEOUT, or PLATFORM_WAIT_ERROR)
 */
PlatformWaitResult platform_wait_single(PlatformHandle_T handle, uint32_t timeout_ms);
 ...
 * @param timeout_ms Timeout in milliseconds (PLATFORM_WAIT_INFINITE for infinite)
 * @return PlatformWaitResult Platform-agnostic wait result
 */
PlatformWaitResult platform_wait_multiple(PlatformHandle_T* handles, uint32_t count, bool wait_all, uint32_t timeout_ms);
