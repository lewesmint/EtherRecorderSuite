...
uint32_t platform_random(void) {
    ...
    if (BCryptGenRandom(NULL, (PUCHAR)&random_number, sizeof(random_number), BCRYPT_USE_SYSTEM_PREFERRED_RNG) != 0) {
        stream_print(stderr, "BCryptGenRandom failed!\n");
        return 0;  // Return 0 on failure
    }
#else
    #if defined(__linux__)
        // Try getrandom() first with GRND_NONBLOCK flag (most secure, guaranteed non-blocking)
        if (getrandom(&random_number, sizeof(random_number), GRND_NONBLOCK) == sizeof(random_number)) {
            return random_number;
        }
    #endif

    // Try /dev/urandom (cryptographically secure, non-blocking)
    ...
    random_number = (uint32_t)(ts.tv_sec ^ ts.tv_nsec);
    random_number ^= (uint32_t)pid;
    random_number ^= (uint32_t)(uintptr_t)tid;
    random_number ^= (uint32_t)(uintptr_t)&random_number; // Add stack address randomness
    
    // Additional mixing using a simple xorshift
