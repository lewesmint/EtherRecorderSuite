...
static SharedMemoryMonitorConfig monitor_config = {
    ...
    .detect_changes = true,        // Only process when data changes
...
static PlatformErrorCode initialize_network(MonitorContext* context) {
    ...
    // Create socket
    result = platform_socket_create(&context->socket_handle, 
                             fwd->use_tcp ? PLATFORM_SOCKET_TCP : PLATFORM_SOCKET_UDP);
    if (result != PLATFORM_ERROR_SUCCESS) {
