...
static const char* const FILE_READ_MODE_STRINGS[] = {
    "FILE_READ_ONCE",
    "FILE_READ_LOOP",
    "FILE_READ_WATCH"
};
...
typedef struct {
    const char* filepath;              ///< Path to the file to read
    const char* foreign_thread_label;  ///< Label of the target thread
...
ThreadConfig* get_file_reader_thread(
    const char* filepath,
